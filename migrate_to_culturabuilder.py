#!/usr/bin/env python3
"""
Migração completa de CulturaBuilder para CulturaBuilder
Complete migration from CulturaBuilder to CulturaBuilder
"""
...
class CulturaBuilderMigration:
    ...
    def __init__(self, root_path: Path, dry_run: bool = False):
        ...
        self.replacements = {
            ...
            # URLs e emails
            "culturabuilder.dev": "culturabuilder.org",
            "contact@culturabuilder.dev": "contact@culturabuilder.org",
            "security@culturabuilder.dev": "security@culturabuilder.org",
            "conduct@culturabuilder.dev": "conduct@culturabuilder.org",
            
            # GitHub
            "culturabuilder-org": "culturabuilder-org",
            "CulturaBuilder_Framework": "CulturaBuilder_Framework",
            "CulturaBuilder_Website": "CulturaBuilder_Website",
        }
        
        # Arquivos para renomear
        self.files_to_rename = {
            "culturabuilder-user-guide.md": "culturabuilder-user-guide.md",
            ".culturabuilder-metadata.json": ".culturabuilder-metadata.json",
            ".culturabuilder-metrics.json": ".culturabuilder-metrics.json",
        }
    ...
    def update_package_files(self):
        ...
        if pyproject_path.exists():
            ...
            content = re.sub(r'name = "culturabuilder"', 'name = "culturabuilder"', content)
            content = re.sub(r'CulturaBuilder', 'CulturaBuilder', content)
            content = re.sub(r'culturabuilder', 'culturabuilder', content)
            
            # Atualiza scripts
            content = content.replace('[project.scripts]', '[project.scripts]')
            content = content.replace('CulturaBuilder = "CulturaBuilder.__main__:main"', 
...
"""
    ...
    def run(self):
        ...
        print("🚀 Iniciando migração CulturaBuilder → CulturaBuilder")
...
def main():
    parser = argparse.ArgumentParser(
        description="Migra CulturaBuilder para CulturaBuilder"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
